...
export default async function handler(req, res) {
  ...
  // Giả sử endpoint SpeechAce như sau (thay đổi nếu cần theo tài liệu của SpeechAce)
  const apiUrl = `https://api.speechace.com/api/scoring/text/v0.1/json/?key=${apiKey}&dialect=general`;
  
  const payload = {
    audio_data: audio,
    reference_text: text,
    // Nếu cần thêm tham số (ví dụ: version, accent, ...) thì thêm vào đây theo tài liệu SpeechAce.
  };
